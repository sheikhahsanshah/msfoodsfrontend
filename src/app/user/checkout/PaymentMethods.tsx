...
export function PaymentMethods({ onSelect, initial = "" }: PaymentMethodsProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const bankList = json.banks || []

        setBanks(bankList)

        if (bankList.length > 0) {
          setSelected(bankList[0].bank_code)
          onSelect(bankList[0].bank_code)
        }
      } catch (err) {
